         ...
         if (!error) {
           if (!result || [result isKindOfClass:[NSString class]] ||
             ...
             returnValue = [result description];
               ...
               NSLog(@"WebLog failed evaluating JavaScripterror = %@", error);
               // mychange
               // 可忽略 js方法为stopLoading 的调用异常，这个方法是用来停止加载的，如果有些URL没有该方法，调用这个方法会报错，但是不影响程序
               // 为了后续可能会使用到，不针对某一个链接做处理,只针对stopLoading方法做处理,(目前只有/home/house/postselect/ 在使用)
               NSString *errorStr = [NSString stringWithFormat:@"%@", error];
               if (![errorStr containsString:@"stopLoading"]) {
